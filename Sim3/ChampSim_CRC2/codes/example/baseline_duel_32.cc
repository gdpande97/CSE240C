...
set<int> lime_sample_set; //sampled set for LIME for dueling, count 32 as of now
set<int> ship_duel_sample; //sampled set for SHIP++ for dueling, count 32
...
//
    ...
    for(int i=0; i<duel_sets_num; i++){
        int lime_set_num = rand()%64;
        lime_sample_set.insert(constituency + lime_set_num);
        if(constituency%128 == 0  && sample_set.size() <sample_sets_num) {
    ...
    for(int i = 0; i < NUM_LEADER_SETS; i++) {
        ...
        while(lime_sample_set.count(ship_consti + ship_set_num)) {
            ship_set_num = rand()%32;
	    cout << "Inside while loop";
        }
    	if(ship_sample_set.size() < NUM_LEADER_SETS)
            ship_sample_set.insert(ship_consti+ ship_set_num);
        if(i%2 == 0) {
            ship_duel_sample.insert(ship_consti + ship_set_num);
        }
	ship_consti += 32;
    }
...
void UpdateReplacementState (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit) {
    ...
    } else if (ship_duel_sample.find(set) != ship_duel_sample.end()) {
        if(hit == 0)
            dir = 0;        
...
void PrintStats()
{
    ...
    cout<<"Ship samples: size is "<<ship_sample_set.size()<<endl;
    for(itr=ship_sample_set.begin(); itr != ship_sample_set.end(); itr++) {
	    cout<<*itr<< ", ";
    }
    cout<<endl;
    cout<<"Ship duel samples: size is "<<ship_duel_sample.size()<<endl;
    for(itr=ship_duel_sample.begin(); itr != ship_duel_sample.end(); itr++) {
	    cout<<*itr<< ", ";
    }
    cout<<endl;
    cout<<"Lime duel samples: size is "<<lime_sample_set.size()<<endl;
    for(itr=lime_sample_set.begin(); itr != lime_sample_set.end(); itr++) {
	    cout<<*itr<< ", ";
    } 
    cout<<endl;
    cout<<"Lime samples: size is "<<sample_set.size()<<endl;
