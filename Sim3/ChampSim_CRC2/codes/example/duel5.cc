...
//
    ...
    for(int i=0; i<duel_sets_num; i++){
        ...
        int ship_set_num = rand()%15;
        while(ship_set_num == lime_set_num) {
            ship_set_num = rand()%15;
        }
        ship_duel_sample.insert(constituency + ship_set_num);
	if(constituency%32 == 0 && ship_sample_set.size()<=NUM_LEADER_SETS) {
