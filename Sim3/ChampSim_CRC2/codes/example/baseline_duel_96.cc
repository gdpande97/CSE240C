...
//
    ...
    for(int i=0; i<duel_sets_num; i++){
        int lime_set_num = rand()%21 ;
        lime_sample_set.insert(constituency + lime_set_num);
        if(constituency%126 == 0  && sample_set.size() < sample_sets_num) {
    ...
    int ship_consti = 0;
    for(int i = 0; i < duel_sets_num; i++) {
	int ship_set_num = rand()%21;
	while(lime_sample_set.count(ship_consti + ship_set_num)) {
	    ship_set_num = rand()%21;
	}
	ship_duel_sample.insert(ship_consti + ship_set_num);
	if((ship_consti%42==0 || ship_consti%63 == 0) && ship_sample_set.size() < NUM_LEADER_SETS ){
