...
void InitReplacementState() {
    ...
    int duel_sets_num = 128 * NUM_CORE;
//    if(cfg == 1 || cfg == 2) {
...
//
    ...
    int ship_consti = 0;
    for(int i = 0; i < duel_sets_num; i++) {
	int ship_set_num = rand()%16;
	while(lime_sample_set.count(ship_consti + ship_set_num)) {
	    ship_set_num = rand()%16;
	}
	ship_duel_sample.insert(ship_consti+ship_set_num);
        if(ship_consti%32 == 0 && ship_sample_set.size()<NUM_LEADER_SETS) {
