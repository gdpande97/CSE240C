...
{
    uint16_t pc0 = pc & 0x7fff;
    uint8_t bit32 = (pc & 0x100000000) >> 32;
    uint8_t bit40 = (pc & 0x10000000000) >> 40;
    uint16_t hash = pc0 + (bit40 <<17) + (bit32<<16) ;    
    return hash;
}

uint64_t hash40(uint64_t tag_addr)
{
    uint64_t top8 = (tag_addr<<(6+LLC_SETS_LOG2)) >> 56;
    uint64_t hash = (top8<<32) + (tag_addr&0xffffffff);
    return hash;
}
...
*/
        ...
        // ---- Update OV HV
        pair<uint64_t, uint64_t> paddr_PC (hash40(tag_addr), hash18(PC));
