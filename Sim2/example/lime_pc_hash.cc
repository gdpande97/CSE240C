...
uint32_t hash22(uint64_t pc)
{
    uint16_t pc0 = pc & 0xfffff;
    uint8_t bit32 = (pc & 0x100000000) >> 32;
    uint8_t bit40 = (pc & 0x10000000000) >> 40;
    uint16_t hash = pc0 + (bit40 <<22) + (bit32<<21) ;    
    return hash;
}
...
{
    uint32_t target = hash22(pc); //hash1 return 16 bit
...
*/
        ...
        // ---- Update OV HV
        pair<uint64_t, uint64_t> paddr_PC (hash37(tag_addr), hash22(PC));
